...
function draw() {
	background(255);
	text("Transmission probability: " + rate, 10, height - 95);
	text("Population movement rate: " + velSlider.value() / 100, 10, height - 65);
	text("Infection radius: " + infectionRadius.value(), 10, height - 35);
	var immuneText;
	...
	text("Immunity: " + immuneText, 40, height - 220);
	text("Death: " + deathText, 40, height - 240);
	...
	if (playing) {
		...
		text("Press Space to engage simulation.", width / 2 - 100, height / 2);
	}
	graph.show();
	legend();
	currCount = 0;
...
function legend() {
	...
	var positionY = 200;
	var positionX = 40;
	text("Uninfected", positionX, height - positionY + 2);
	fill(0, 255, 0);
	...
	fill(0);
	text("Infected", positionX, height - positionY + 43);
	fill(255, 0, 0);
	ellipse(positionX - 20, height - positionY + 40, 10, 10);

	fill(0);
	text("Dead", positionX, height - positionY + 63);
	fill(0, 0, 0);
	ellipse(positionX - 20, height - positionY + 60, 10, 10);

	fill(0);
	text(
		"Total Infected (Current and Recovered)",
		positionX,
		height - positionY + 83
	);
	stroke(255, 0, 255);
